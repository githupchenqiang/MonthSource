...
/**上传或者下载的进度,Progress.completedUnitCount:当前的大小,Progress.totalUnitCount的Block*/
typedef void(^HttpProgress)(NSProgress *progress);

/**网络状态的Block*/
typedef void(^NetWorkSttus)(NetworkStatusType status);

@interface CQ_CacheNetWorkHelper : NSObject
...
/**
 ...
 @param URL 请求地址
 @param parameters 请求参数
 @param responseCache 请求数据的回调
 @param success 请求成功的回调
 @param failure 请求失败的回调
 @return 返回的对象可取消请求调用cancel方法
 */
+ (__kindof NSURLSessionTask *)GET:(NSString *)URL
                        parameters:(NSDictionary *)parameters
...
/**
 Post请求 自动缓存 缓存策略 URL + Page + type
