...
@implementation CQ_CacheNetWorkHelper
static BOOL _isOpenLog; //是否已开启日志打印
static NSMutableArray *_allSessionTask;
static AFHTTPSessionManager *_sessionManager;


+ (void)networkStatusWithBlock:(NetWorkSttus)networkStatus
{
...
+ (NSURLSessionTask *)GET:(NSString *)URL
            ...
            responseCache:(HttpRequestCache)responseCache
                  ...
                  failure:(HttpRequestFailed)failure {
    //读取缓存
    responseCache ? responseCache([CQ_NetWorkCache httpCacheForURL:URL parameters:parameters]) : nil;
    ...
    NSURLSessionTask *sessionTask = [_sessionManager GET:URL parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        //对数据进行异步缓存
        responseCache ? [CQ_NetWorkCache setHttpCache:responseObject Url:URL parameters:parameters] : nil;
...
+ (NSURLSessionTask *)POST:(NSString *)URL
    ...
    NSURLSessionTask *sessionTask = [_sessionManager POST:URL parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        success ? success(responseObject) : nil;
        //对数据进行异步缓存
        responseCache ? [CQ_NetWorkCache setHttpCache:responseObject Url:URL parameters:responseObject] : nil;
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        PPLog(@"%@",_isOpenLog ? NSStringFormat(@"error = %@",error) : @"PPNetworkHelper已关闭日志打印");
                            ...
                            failure:(HttpRequestFailed)failure
{
    ...
    responseCache ? responseCache([CQ_NetWorkCache httpCacheForURL:URL parameters:pageType]) : nil;
    NSURLSessionTask *sessionTask = [_sessionManager POST:URL parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        success ? success(responseObject) :nil;
        //对数据进行异步缓存
        responseCache ? [CQ_NetWorkCache setHttpCache:responseObject Url:URL parameters:pageType] : nil;
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        PPLog(@"%@",_isOpenLog ? NSStringFormat(@"error = %@",error) : @"PPNetworkHelper已关闭日志打印");
